...
`defineConversation({ kind, events }).component(...)` colocates one official Conversation node definition with its keyed `conversation.chat.node` renderer. `defineClient({ conversations })` registers the definition before the renderer and infers `conversationEvents` plus `slots`; authors no longer maintain a separate lifecycle object and Slot contribution for the same node kind. The component only receives projected data. Match, state folding, replay, sequence ordering, publication, pagination, location, duplicate handling, and disposal remain owned by the official Client assembler and Cordis Fiber.

Conversation components do not receive an implicit Host action bus. Typed writes compose the existing `defineApi`/`useApi` Connection contract, including its AbortSignal and lifecycle behavior. A retained `useApi` or `useQuery` Hook now declares the Client Connection capability and lazily binds contracts by identity inside the current Client Fiber, so `defineClient` does not repeat API contracts; the older `api/apis` fields remain a compatible eager form. The Host must re-resolve live Session or Agent state and own authorization, revision comparison, idempotency, durable outcomes, and explicit business cancellation. Commands are used only when the operation is genuinely a Command, and DSHX does not add a private WebSocket, Session registry, action facet, optimistic state, or automatic retry loop.
