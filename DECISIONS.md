...
rc.8 exposes `cordis_inspect_list`/`cordis_inspect_query` and Host/Client inspect registries inside the Composition, but no supported external CLI or HTTP seam was found. Consequently the default CLI reports `DSHX3201` when no compatibility adapter supplies a provider. Provider connection failures, malformed DTOs, unsupported targets, and absent links use `DSHX3202` through `DSHX3205`; the original cause is retained for `--verbose`. Inspect does not generate TypeScript or provide Slot type completion: `defineSlot()` remains governed by official provider declaration merging. The next `dshx add ui` stage may consume this runtime result to generate source.
...
## 2026-08-22: Services and Events Inspect remain adapter-gated
...
The rc.8 adapter intentionally advertises only `slots` and `tools`. Therefore Services/Events requests return `DSHX3204` before invoking an injected Provider, rather than returning an empty or fabricated Catalog. A future adapter may add these targets only when an official Runtime Provider exists and passes a real fixture smoke test; Provider absence, connection failure, malformed DTOs, and missing Profile links retain the existing `DSHX3201` through `DSHX3205` semantics. Inspect remains read-only and does not start DSH, mutate Profile links, or write cache files.
