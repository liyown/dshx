...
Client rebuilds leave DSH running and rely exclusively on the selected adapter's native HMR. A Host rebuild under the default `manual` policy sets `hostRestartRequired`; `auto` serializes stop/start operations so only one DSH child is active. Web processes receive `--no-open` unless the caller supplies an open policy. The process inherits the environment, uses the project root as cwd, and runs through the selected local or PATH DSH executable with `--profile <profile>`.
...
DSHX follows the standard plugin-tooling model of peer dependency ranges plus compatibility testing. The first adapter represents the DSH `0.1` protocol generation and declares `>=0.1.0-rc.8 <0.2.0`; `0.1.0-rc.8` and `0.1.1-rc.2` are verified through isolated Host/Client, Profile, child-plugin, Agent-backed Inspect, and Bridge smoke tests. Other versions in the range remain `compatible-range` warnings until smoke-tested. DSH `0.2` is treated as a possible breaking boundary because this project is still pre-1.0, so it requires a new adapter rather than silently reusing the `0.1` protocol.
...
## 2026-08-22: Offline Catalogs are explicit package metadata, not runtime fallback

The compatibility adapter may declare a static Catalog only when an installed DSH or official provider exposes a stable package-metadata seam. `dshx catalog slots|tools|services|events` is read-only and labels results with `source: "offline"`, the detected DSH version, adapter id, and protocol generation. It never connects to a running Composition, invokes a Profile command, starts DSH, infers capabilities from TypeScript source, or writes `.dshx/cache/catalog.json`. `dshx inspect` remains runtime-only and never falls back to this Catalog; `dshx add ui` likewise requires its live Slot tree and exact contract unless a future command explicitly opts into offline generation.

The rc.8 adapter currently declares no verified package-metadata targets, so Catalog commands return `DSHX3301` instead of fabricated empty data. Provider failures use `DSHX3302`, malformed metadata uses `DSHX3303`, and the original cause is retained only for verbose output. A future adapter must provide real metadata and smoke coverage before declaring targets supported.

The internal manifest repair plan is deliberately narrower than a fix command. It previews and atomically applies only missing fields whose values are determined by the selected adapter and resolved source faces: exports, `dsh.bundle.patch` when the patch file exists, and required `dsh.client` defaults. Existing conflicting values, missing patch files, malformed containers, source code, dependencies, and Profile links are never guessed or overwritten. `dshx check --fix` remains a later user-facing stage.
