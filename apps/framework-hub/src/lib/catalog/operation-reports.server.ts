...
export async function latestOperationReport(binding: D1Database) {
...
export async function publishOperationReport(
  ...
  const existing = await binding
    .prepare("select payload_hash from hub_operation_reports where run_id=?")
    .bind(input.runId)
...
async function reportById(binding: D1Database, runId: string) {
  const row = await binding
    .prepare(
      `select run_id,started_at,completed_at,outcome,body_en,body_zh,payload_hash,created_at
