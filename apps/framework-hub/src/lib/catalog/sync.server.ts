...
const promotionStatements = [
  ...
  `insert into plugin_releases(id,plugin_id,version,channel,git_tag,commit_sha,compatibility_range,compatibility_source,release_notes_url,deprecated,published_at,updated_at)
   select json_extract(i.payload_json,'$.plugin.id')||':release:'||json_extract(r.value,'$.version'),json_extract(i.payload_json,'$.plugin.id'),json_extract(r.value,'$.version'),json_extract(r.value,'$.channel'),json_extract(r.value,'$.gitTag'),json_extract(r.value,'$.commitSha'),json_extract(r.value,'$.compatibilityRange'),json_extract(r.value,'$.compatibilitySource'),json_extract(r.value,'$.releaseNotesUrl'),json_extract(r.value,'$.deprecated'),coalesce(strftime('%s',json_extract(r.value,'$.publishedAt'))*1000,unixepoch()*1000),unixepoch()*1000 from catalog_sync_items i,json_each(i.payload_json,'$.releases') r where i.run_id=? and i.validation_status='accepted'`,
