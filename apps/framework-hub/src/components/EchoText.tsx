...
const EchoText: React.FC<EchoTextProps> = ({
  ...
  const initialVector = directionVectors[direction] || directionVectors.right;
  const initialOffset = clamp(Number(offset) || 0, 0, 120);
  const initialFade = clamp(Number(fade) || 0.64, 0.1, 0.95);
  const entranceEnabled = mode === "entrance" || mode === "both";

  const initialTransform = (index: number) => {
    const amount = entranceEnabled ? initialOffset * (index + 0.35) : 0;
    return `translate3d(${(initialVector.x * amount).toFixed(3)}px, ${(initialVector.y * amount).toFixed(3)}px, 0)`;
  };
  ...
  return (
    <span ref={rootRef} className={`echo-text ${className}`.trim()} style={rootStyle}>
          ...
          <span
            ...
            style={{
              color: tint
                ...
                : color,
              opacity: entranceEnabled ? Math.pow(initialFade, index) : 0,
      ...
      <span
        ...
        style={{ transform: initialTransform(0) }}
      >
        {text}
      </span>
    </span>
