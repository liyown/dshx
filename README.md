...
`dshx catalog <slots|tools|services|events>` is a separate read-only offline query. It only uses static package metadata explicitly exposed by the selected compatibility adapter and labels every result `source: "offline"`; it never connects to the running Composition, starts DSH, reads Profile state, or writes `.dshx/cache/catalog.json`. The current rc.8 adapter has no verified package-metadata seam, so it returns `DSHX3301` rather than an empty or fabricated Catalog. Runtime `dshx inspect` remains the only command that describes the current Composition, and `add ui` does not consume offline results by default.
...
Manifest checking reports all errors and publishing warnings without rewriting `package.json`, `dshx.config.ts`, or `cordis.patch.yml`. Enabled source faces require exports and DSH metadata for the selected compatibility adapter. Host-only projects omit Client metadata; Client-only projects retain the root Host export for the generated no-op Host artifact. The internal `createManifestRepairPlan()` API can preview and atomically apply only missing, adapter-derived manifest fields; it does not modify source files, dependencies, Profile links, or ambiguous existing values. The public `dshx check --fix` command is still not enabled.
