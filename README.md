...
The public packages are `@becomeopc/dshx` (the compiler, runtime helpers, and `dshx` CLI) and `create-dshx` (the project initializer). Start a new plugin with the project-book workflow:
...
The package exposes five user commands. `dshx build` validates the manifest and builds whichever Host and/or Client entries the project provides, without touching DSH or project metadata. There is no required Full, Host-only, Client-only, or native project mode: conventional DSHX definitions and native named modules can be used independently or together, and `dshx.config.ts` can point each face at any in-project entry (or disable it). `dshx check` performs the same read-only manifest checks plus DSH version, Profile-link, adapter-approved runtime plugin, and Host Inspect Bridge status inspection; `dshx check --fix` can apply only deterministic manifest metadata repairs, while `--dry-run` previews them. Use `--json` for automation. DSHX prefers the project-local `pnpm exec dsh`, then falls back to the official `dsh` on PATH, so plugin developers can debug against an existing user installation without adding DSH to every plugin manifest. `dshx dev` ensures the project is linked through the selected DSH CLI, starts the coordinated watchers, and launches DSH only after the entries that are present build successfully. Raw `dshx dev` sessions pass `--no-open` by default; generated `create-dshx` projects use `dshx dev --open` in their package-manager script so `pnpm dev` opens the browser automatically. In an interactive terminal, `r` restarts DSH and `q` closes the session.
...
Use `dshx add command --name <name>` to scaffold an official slash Command. The generated `src/commands/<name>.ts` uses `defineCommand()` and is attached to `defineHost({ commands })`; `--description`, `--file`, `--dry-run`, and `--json` are supported. Names follow the official lowercase command grammar. The generator does not add a parser or execution layer, install packages, inspect a running Composition, or change Profiles. Existing native Hosts, disabled Hosts, unsafe `commands` shapes, duplicate definitions, write failures, and post-write Manifest errors are handled without partial edits.
...
Projects that provide a Host can import `defineHost`, `defineCommand`, and the official `defineTool` for the selected DSH generation from `@becomeopc/dshx/host` and default-export a definition with `inject`, tools, commands, and direct `setup(ctx)` access to the official Cordis Context. `defineTool` is the exact official function and `defineCommand` preserves the exact official `CommandDefinition`; schemas, execution, parsing, ordering, scope, collisions, cancellation, and lifecycle disposal remain owned by DSH. `defineHost()` preserves the original object; the Host compiler supplies the resolved logical name, merges the `tools` and `commands` dependencies when needed, registers contributions through their official registries in declaration order, and runs setup afterward. Native named Host modules remain supported when direct control is preferred. The Host helpers and thin adapter are inlined into `dist/index.js`, while the official runtime packages remain bare externals resolved by DSH.
...
Unary API cancellation always uses the second call argument so input-bearing and no-input methods have one runtime shape: `api.refresh({ force: true }, { signal })` and `api.get(undefined, { signal })`. `useQuery()` owns its request cancellation, waits while the official Host description is absent, and retries once when a new Host generation becomes available.

For an rc.2 cold-start compatibility matrix, run `pnpm run smoke:rc2`. It exercises several independent combinations of Host/Client definitions and native modules in a temporary root; these are test fixtures, not user-facing project modes. The matrix installs the published `0.1.1-rc.2` DSH packages plus a local dshx tarball from the populated pnpm store, builds each project, exercises Profile linking, runtime Inspect, scaffold idempotency, real Host API calls, official Command registry/parser execution, Client HMR, Host restart, and cleanup, then removes the temporary root. Set `DSHX_KEEP_SMOKE=1` only when preserving a failed run for diagnosis is necessary.
