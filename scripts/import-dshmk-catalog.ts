...
import { canonicalJson, parsePluginObservation, type PluginObservationV1 } from '../packages/framework-hub-cli/dist/contracts.js'
import { curatePlugin, getPlugin, publishReport, upsertPlugins } from '../packages/framework-hub-cli/dist/operations.js'
...
function optionValues(name: string): string[] {
  return process.argv.flatMap((value, index, values) => (value === name && values[index + 1] ? [values[index + 1]] : []))
}
...
function productionCatalog(): ExistingCatalog {
  ...
  if (!parsed[0]?.success) throw new Error('Production catalog identity query did not succeed.')
  const rows = parsed[0].results ?? []
  const identifiedRows = rows.filter((row): row is typeof row & { identity_key: string } => Boolean(row.identity_key))
  return {
    curatedIdentities: new Set(identifiedRows.filter(row => row.curated === 1).map(row => row.identity_key)),
    directIdentities: new Set(identifiedRows.filter(row => row.direct === 1).map(row => row.identity_key)),
    identities: new Set(identifiedRows.map(row => row.identity_key)),
    importedIdentities: new Set(identifiedRows.filter(row => row.imported === 1).map(row => row.identity_key)),
    packageNames: new Set(rows.map(row => row.package_name?.toLowerCase()).filter((value): value is string => Boolean(value))),
    repositories: new Set(rows.map(row => repositoryName(row.repository_url)).filter((value): value is string => Boolean(value))),
    shortSeoIdentities: new Set(identifiedRows.filter(row => row.short_seo === 1).map(row => row.identity_key)),
    pluginIds: new Map(identifiedRows.flatMap(row => (row.plugin_id ? [[row.identity_key, row.plugin_id] as const] : []))),
    publishedIdentities: new Set(identifiedRows.filter(row => row.published === 1).map(row => row.identity_key)),
  }
}

function productionBookmark(): string {
  const output = execFileSync('pnpm', ['exec', 'wrangler', 'd1', 'time-travel', 'info', 'dshx-framework-hub', '--json'], {
    cwd: hubDirectory,
    encoding: 'utf8',
...
function alreadyPresent(candidate: SourceRepository, current: ExistingCatalog) {
  ...
  if (install.source === 'npm' && current.packageNames.has(npmSpecifier(install.specifier ?? '').name)) return true
...
async function retry<T>(label: string, action: () => Promise<T>, attempts = 3): Promise<T> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = error
      if (attempt < attempts) await new Promise(resolve => setTimeout(resolve, Math.min(30_000, 1_000 * 2 ** (attempt - 1))))
    }
  }
...
async function mapConcurrent<T, U>(items: T[], limit: number, action: (item: T, index: number) => Promise<U>): Promise<U[]> {
...
function executableCandidate(candidate: SourceRepository): InstallCandidate | undefined {
  const matches = candidate.install?.candidates?.filter(item => item.executable === true) ?? []
  return matches.length === 1 ? matches[0] : undefined
}

function identityFor(candidate: SourceRepository): string {
  const install = executableCandidate(candidate)
  if (!install?.specifier) throw new Error(`${candidate.fullName} has no exact executable target.`)
  return install.source === 'npm' ? `npm:${npmSpecifier(install.specifier).name}` : `github:${String(candidate.repositoryId)}:`
}

function operationIdentity(candidate: SourceRepository) {
  const identity = identityFor(candidate)
  if (identity.startsWith('npm:')) return { kind: 'npm' as const, packageName: identity.slice(4) }
  return {
...
function sourceRepositoryFacts(candidate: SourceRepository) {
  return {
    ...
    fullName: candidate.fullName,
    ...(bounded(candidate.defaultBranch, 255) ? { defaultBranch: bounded(candidate.defaultBranch, 255) } : {}),
    ...(bounded(candidate.description, 2_000) ? { description: bounded(candidate.description, 2_000) } : {}),
    ...(validUrl(candidate.homepage) ? { homepageUrl: validUrl(candidate.homepage) } : {}),
    topics: (candidate.topics ?? []).filter(Boolean).slice(0, 100),
    ...(bounded(candidate.language, 100) ? { primaryLanguage: bounded(candidate.language, 100) } : {}),
    ...(bounded(candidate.license, 100) ? { licenseSpdx: bounded(candidate.license, 100) } : {}),
...
function discoveryObservation(candidate: SourceRepository, generatedAt: string): PluginObservationV1 {
  const install = executableCandidate(candidate)!
  ...
  const exactNpm = install.source === 'npm' ? npmSpecifier(install.specifier ?? '') : null
  const npmVersion = exactNpm?.selector && /^v?\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(exactNpm.selector) ? exactNpm.selector.replace(/^v/, '') : undefined
...
async function graphRepositories(candidates: SourceRepository[]): Promise<Map<string, GraphRepository>> {
  ...
  for (const batch of chunks(candidates, 35)) {
    ...
    if (body.errors?.length) throw new Error(`GitHub GraphQL returned errors: ${JSON.stringify(body.errors)}`)
...
function graphRepositoryFacts(repository: GraphRepository) {
  const topics = repository.repositoryTopics?.nodes?.map(node => node?.topic?.name).filter((value): value is string => Boolean(value)) ?? []
  return {
    githubId: String(repository.databaseId),
    fullName: repository.nameWithOwner,
    ...(bounded(repository.defaultBranchRef?.name, 255) ? { defaultBranch: bounded(repository.defaultBranchRef?.name, 255) } : {}),
    ...(bounded(repository.description, 2_000) ? { description: bounded(repository.description, 2_000) } : {}),
    ...(validUrl(repository.homepageUrl) ? { homepageUrl: validUrl(repository.homepageUrl) } : {}),
    topics,
    ...(bounded(repository.primaryLanguage?.name, 100) ? { primaryLanguage: bounded(repository.primaryLanguage?.name, 100) } : {}),
    ...(bounded(repository.licenseInfo?.spdxId, 100) ? { licenseSpdx: bounded(repository.licenseInfo?.spdxId, 100) } : {}),
...
function publisherFacts(repository: GraphRepository) {
  return {
    ...
    kind: repository.owner.__typename === 'Organization' ? ('organization' as const) : ('user' as const),
    avatarUrl: repository.owner.avatarUrl,
    profileUrl: repository.owner.url,
  }
}

async function rawFile(candidate: SourceRepository, paths: string[]): Promise<{ content: string; path: string; url: string } | undefined> {
  ...
  for (const path of paths) {
    ...
    if (content.trim() && content.length <= 200_000) return { content, path, url }
  }
  return undefined
}

async function packageReadme(packageName: string, version: string): Promise<{ content: string; path: string; url: string } | undefined> {
  const encodedName = packageName
    .split('/')
    .map(part => encodeURIComponent(part))
    .join('/')
  for (const path of ['README.md', 'readme.md', 'Readme.md', 'README.MD', 'README.zh-CN.md']) {
...
function packageRepositoryUrl(manifest: Record<string, unknown>): string | undefined {
...
function packageFacts(manifest: Record<string, unknown>, fallbackName?: string) {
  ...
  return {
    name,
    version,
    ...(bounded(manifest.description, 2_000) ? { description: bounded(manifest.description, 2_000) } : {}),
    ...(bounded(manifest.license, 100) ? { license: bounded(manifest.license, 100) } : {}),
    ...(validUrl(manifest.homepage) ? { homepageUrl: validUrl(manifest.homepage) } : {}),
    ...(repositoryUrl ? { repositoryUrl } : {}),
    ...(keywords?.length ? { keywords } : {}),
    ...(manifest.deprecated === true || typeof manifest.deprecated === 'string' ? { deprecated: true } : {}),
  }
}

function compatibility(manifest: Record<string, unknown>) {
  const peerDependencies =
    manifest.peerDependencies && typeof manifest.peerDependencies === 'object' ? (manifest.peerDependencies as Record<string, unknown>) : {}
  const engines = manifest.engines && typeof manifest.engines === 'object' ? (manifest.engines as Record<string, unknown>) : {}
  const declaredRange = bounded(peerDependencies['deepseek-harness'] ?? engines['deepseek-harness'] ?? engines.dsh, 200)
  return declaredRange ? { declaredRange, source: 'peer-dependency' as const } : undefined
}
...
function readmeEvidence(readme: string): {
  ...
  const flushBullet = () => {
    ...
    if (capabilityEvidence(value) && value.length <= 700 && !/install|安装|clone|license|许可证/i.test(value))
  ...
  for (const raw of lines) {
    ...
    if (/^(?:\[!|<img|https?:\/\/|npm |pnpm |yarn |dsh plugin)/i.test(trimmed)) {
      flush()
      flushBullet()
      continue
    }
    if (/^#{1,6}\s/.test(trimmed)) {
      flush()
      flushBullet()
      preferredSection = /(?:features?|highlights?|capabilit|功能|特性|主要能力)/iu.test(trimmed)
      continue
    }
    if (/^[-+*•]\s+/.test(trimmed)) {
      flush()
  ...
  const fallback = paragraphs.filter(value => value !== intro && capabilityEvidence(value)).slice(0, 4)
...
function buildShortDescription(input: { description: string; features: string[]; locale: 'en' | 'zh' }): string {
  let summary = trimText(input.description, 240)
  for (const feature of input.features) {
    const substantive =
      input.locale === 'en'
        ? summary.length >= 72 && (summary.match(/[A-Za-z][A-Za-z0-9'-]*/g) ?? []).length >= 10
        : summary.length >= 36 && (summary.match(/[\u3400-\u9fff]/g) ?? []).length >= 18
    ...
    if (comparableSummary.includes(comparableDetail) || comparableDetail.includes(comparableSummary)) continue
    summary = trimText(`${summary.replace(/[.!?。！？；;：:]$/u, '')}${input.locale === 'zh' ? '；' : ' — '}${detail}`, 240)
  }
  return summary
}

async function translateParts(parts: string[], from: 'en' | 'zh', to: 'en' | 'zh', cache: ImportCache): Promise<string[]> {
  ...
  if (!translated) {
    ...
    const response = await fetchResponse('https://translate.googleapis.com/translate_a/single', {
      method: 'POST',
      headers: {
    ...
    translated = (payload[0] ?? []).map(piece => String(piece?.[0] ?? '')).join('')
    cache.translations ??= {}
    cache.translations[key] = translated
    saveCache(cache)
  }
  const matches = [...translated.matchAll(/\[(\d+)]\s*([^\[]+)/g)]
  if (matches.length !== parts.length) throw new Error('Translation response did not preserve item markers.')
  const output = new Array<string>(parts.length)
  for (const match of matches) output[Number(match[1])] = match[2]!.trim()
  if (output.some(value => !value)) throw new Error('Translation response omitted an item.')
  return output
}

async function localizeParts(parts: string[], cache: ImportCache): Promise<Record<'en' | 'zh', string[]>> {
  ...
  for (const sourceLocale of ['en', 'zh'] as const) {
    ...
    const translated = await translateParts(sourceParts, sourceLocale, targetLocale, cache)
...
function buildOverview(input: { description: string; features: string[]; installSpec: string; locale: 'en' | 'zh'; ref: string }): string {
...
async function prepareCandidate(candidate: SourceRepository, repository: GraphRepository | undefined, cache: ImportCache): Promise<PreparedCandidate> {
  try {
    if (!repository)
      return {
        candidate,
    ...
    if (!sourceSha) return { candidate, reason: 'Validated source commit is missing.' }
    ...
    if (install.source === 'npm') {
      ...
      readme = await rawFile(candidate, ['README.md', 'readme.md', 'Readme.md', 'README.MD', 'README.zh-CN.md'])
    ...
    const readmeSource = readme?.url ?? (source.kind === 'npm' ? source.url : `https://github.com/${repository.nameWithOwner}`)
    ...
    const directObservation = parsePluginObservation({
      ...
      facts: {
        ...
        ...(compatibility(manifest) ? { compatibility: compatibility(manifest) } : {}),
        metrics: {
    ...
    const translated = await localizeParts([rawDescription, ...sourceFeatures], cache)
    const localized = {
    ...
    const directUrls = [repository.url, readme.url, source.url, `https://dshmk.com/plugins/${candidate.slug}`]
    const curation = {
      displayName: {
      ...
      tags: [...new Set((candidate.topics ?? []).map(value => value.toLowerCase()).filter(value => value.length <= 64))].slice(0, 30),
    ...
    return {
      candidate,
      reason: message.includes('translate.googleapis.com') ? 'Translation service unavailable.' : message,
    }
  }
}

function resultRows(envelope: { data: unknown }): Array<Record<string, unknown>> {
  if (!envelope.data || typeof envelope.data !== 'object') return []
  const results = (envelope.data as { results?: unknown }).results
  return Array.isArray(results) ? results.filter((value): value is Record<string, unknown> => Boolean(value) && typeof value === 'object') : []
}
...
function isIdentityConflict(result: Record<string, unknown>): boolean {
  ...
  return Boolean(error) && typeof error === 'object' && 'code' in error && error.code === 'observation_identity_conflict'
}

async function upsertBatches(observations: PluginObservationV1[], batchSize: number, dryRun: boolean) {
  const results: Array<Record<string, unknown>> = []
  for (const [index, batch] of chunks(observations, batchSize).entries()) {
    const envelope = await retry(`observation batch ${index + 1}`, () => upsertPlugins(hub, { observations: batch }, dryRun), 8)
    const batchResults = resultRows(envelope)
    results.push(...batchResults)
    process.stdout.write(`Observation batch ${index + 1}/${Math.ceil(observations.length / batchSize)}: ${JSON.stringify(summarizeStatuses(batchResults))}\n`)
    const rejected = batchResults.filter(result => result.status === 'rejected')
    if (rejected.length) process.stdout.write(`Rejected observations: ${JSON.stringify(rejected)}\n`)
...
async function main(): Promise<void> {
  ...
  if (catalog.schemaVersion !== 1 || !Array.isArray(catalog.repositories)) throw new Error('Unexpected public catalog schema.')
  ...
  if (repairShortSeo || recurate) {
    selected = verified.filter(candidate => recurate || currentCatalog.shortSeoIdentities.has(identityFor(candidate)))
  } else if (apply && cache.selectedIdentities?.length && !cache.completed) {
    const saved = new Set(cache.selectedIdentities)
    selected = verified.filter(candidate => saved.has(identityFor(candidate)))
  } else {
    selected = verified.filter(candidate => !alreadyPresent(candidate, currentCatalog))
    selected.sort((left, right) => (right.stars ?? 0) - (left.stars ?? 0) || left.fullName.localeCompare(right.fullName))
  ...
  if (requestedIdentities.length) {
    ...
    selected = selected.filter(candidate => requested.has(identityFor(candidate)))
  }
  if (!selected.length) {
  ...
  const prepared = await mapConcurrent(selected, concurrency, async (candidate, index) => {
    const result = await prepareCandidate(candidate, repositories.get(candidate.fullName.toLowerCase()), cache)
    if ((index + 1) % 25 === 0 || index + 1 === selected.length) process.stdout.write(`Prepared ${index + 1}/${selected.length}\n`)
    return result
  })
  const discovery = selected
    .filter(candidate => !currentCatalog.importedIdentities.has(identityFor(candidate)))
    .map(candidate => discoveryObservation(candidate, catalog.generatedAt))
  const directPrepared = prepared.flatMap(item => (item.directObservation ? [item.directObservation] : []))
  const direct = directPrepared.filter(observation => {
    const identity = observationIdentity(observation)
    return (!currentCatalog.directIdentities.has(identity) || recurate) && !blockedIdentities.has(identity)
  })
  ...
  const pendingCurationCandidates = curatablePrepared.filter(
    item =>
      ...
      recurate ||
      (repairShortSeo && currentCatalog.shortSeoIdentities.has(identityFor(item.candidate))),
  )
  const validation = {
  ...
  if (!apply) {
    ...
    const dryDirect = direct.length ? await upsertBatches(direct, 10, true) : []
    process.stdout.write(
  ...
  const directResults = direct.length ? await upsertBatches(direct, 10, false) : []
  for (const result of directResults) {
    if (isIdentityConflict(result) && typeof result.identity === 'string') blockedIdentities.add(result.identity)
  }
  ...
  const curatableByPlugin = new Map<string, (typeof curatablePrepared)[number]>()
  for (const item of curatablePrepared) {
    ...
    if (
      (!afterDirectCatalog.publishedIdentities.has(identity) || recurate || (repairShortSeo && afterDirectCatalog.shortSeoIdentities.has(identity))) &&
      !curatableByPlugin.has(pluginId)
    )
      curatableByPlugin.set(pluginId, item)
  }
  const curatable = [...curatableByPlugin.entries()].map(([pluginId, item]) => ({ pluginId, item }))
  const curationResults = await mapConcurrent(curatable, Math.min(concurrency, 6), async ({ pluginId, item }, index) => {
    const identity = identityFor(item.candidate)
    try {
      let curated: Awaited<ReturnType<typeof curatePlugin>> | undefined
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        const current = await retry(`curation state ${identity}`, () => getPlugin(hub, pluginId), 8)
        const state = current.data as { revision?: number }
        try {
          curated = await retry(`curation ${identity}`, () => curatePlugin(hub, pluginId, item.curation, state.revision), 8)
          break
        } catch (error) {
          const code = error && typeof error === 'object' && 'issue' in error && (error as { issue?: { code?: unknown } }).issue?.code
          if (code !== 'revision_conflict' || attempt === 2) throw error
        }
      }
      if (!curated) throw new Error('Curation did not return a result.')
      const verifiedPlugin = await retry(`curation verification ${identity}`, () => getPlugin(hub, pluginId), 8)
      const verification = verifiedPlugin.data as {
        needs?: string[]
        state?: string
      }
      if (verification.state !== 'published' || (verification.needs?.length ?? 0) > 0)
        return {
          identity,
          status: 'incomplete',
      ...
      if ((index + 1) % 20 === 0 || index + 1 === curatable.length) process.stdout.write(`Curated ${index + 1}/${curatable.length}\n`)
      return {
        identity,
  ...
  const rejectedObservations = [...discoveryResults, ...directResults].filter(result => result.status === 'rejected').length
  const incomplete = curationResults.filter(result => result.status === 'incomplete').length
  const rejectedCurations = curationResults.filter(result => result.status === 'rejected').length
  const unsuccessfulCurations = curationResults.filter(result => result.status === 'incomplete' || result.status === 'rejected')
  if (unsuccessfulCurations.length) process.stdout.write(`Unsuccessful curations: ${JSON.stringify(unsuccessfulCurations)}\n`)
  const finalCatalog = productionCatalog()
  const selectedIdentities = selected.map(identityFor)
  const imported = selectedIdentities.filter(identity => finalCatalog.importedIdentities.has(identity)).length
  const directComplete = selectedIdentities.filter(identity => finalCatalog.directIdentities.has(identity)).length
  const directPreparedIdentities = new Set(directPrepared.map(observationIdentity))
  const blockedDirect = [...directPreparedIdentities].filter(identity => blockedIdentities.has(identity) && !finalCatalog.directIdentities.has(identity)).length
  const directSatisfied = [...directPreparedIdentities].filter(
    identity => finalCatalog.directIdentities.has(identity) || blockedIdentities.has(identity),
  ).length
  const publishedIdentities = selectedIdentities.filter(identity => finalCatalog.publishedIdentities.has(identity))
  const remainingShortSeo = selectedIdentities.filter(identity => finalCatalog.shortSeoIdentities.has(identity)).length
  ...
  if (!requestedIdentities.length && !repairShortSeo && !recurate) cache.completed = outcome === 'completed'
  saveCache(cache)
  process.stdout.write(
