...
[English](./README.md) · [文档](./docs/index.md) · [Framework Hub](https://dshx.io/zh-CN) · [路线图](./ROADMAP.md)
...
最终 tree-shaking 后仍保留的 `useApiQuery` 会自动声明 `connection` 能力；Client 不重复声明 API contract 或 Settings 所有权。
...
| `/client`                      | API Candidate          | Client 定义、Slots、API/Settings Hooks          |
...
| [Client 与 Slot](./docs/guides/client.md)     | `defineClient`、`defineSlot`、官方 props、能力推断          |
...
当前 `protocol-1` adapter 覆盖 `>=0.1.0-rc.8 <0.2.0-0`；真实运行时验证边界为 `0.1.0-rc.8` 和 `0.1.1-rc.2`。插件公开支持范围写入 `peerDependencies`，本地开发使用一个具体 `devDependencies` 版本。
