...
async function setup(host: 'define' | 'native' | 'missing' | 'disabled'): Promise<{ root: string; project: ResolvedDshxConfig; cleanup: () => Promise<void> }> {
  ...
  if (host === 'define')
    await writeFile(resolve(root, 'src/host.ts'), "import { defineHost } from '@becomeopc/dshx/host'\n\nexport default defineHost({ setup() {} })\n")
  ...
  return {
    root,
    project: project(root, host === 'missing' || host === 'disabled' ? undefined : resolve(root, 'src/host.ts'), configFile),
...
describe('add command scaffold', () => {
  it('generates the smallest official Command and appends it to defineHost', async () => {
    ...
    try {
      ...
      expect(host).toContain('from "./commands/status.js"')
  ...
  it('rejects invalid names, native Hosts, disabled Hosts, and rolls back checker failures', async () => {
    ...
    try {
      ...
      const failed = await createCommandScaffold(
        { project: value.project, name: 'status' },
    ...
    try {
      expect((await createCommandScaffold({ project: native.project, name: 'status' })).diagnostics[0]?.code).toBe('DSHX6504')
    } finally {
      await native.cleanup()
    }
    const disabled = await setup('disabled')
    try {
      expect((await createCommandScaffold({ project: disabled.project, name: 'status' })).diagnostics[0]?.code).toBe('DSHX6503')
    } finally {
