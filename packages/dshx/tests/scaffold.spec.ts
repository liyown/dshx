...
const compatibility = {
  id: 'dsh-0.1', protocolGeneration: '0.1', version: '0.1.0-rc.8', dshRange: '>=0.1.0-rc.8 <0.2.0-0', verified: { minimum: '0.1.0-rc.8', latest: '0.1.0-rc.8' }, verifiedVersions: ['0.1.0-rc.8'],
...
function project(root: string, clientEntry: string | undefined, manifest: Record<string, unknown>): ResolvedDshxConfig {
  return {
    root, packageFile: resolve(root, 'package.json'), configDependencies: [], packageId: '@demo/plugin', name: '@demo/plugin',
    ...(clientEntry === undefined ? {} : { clientEntry }), hostEntry: resolve(root, 'src/host.ts'), outDir: resolve(root, 'dist'), profile: 'web',
    dev: { hostRestart: 'manual' }, build: { sourcemap: true }, compatibility: { allowUnsupported: false }, manifest,
  }
}
...
async function setup(full: boolean): Promise<{ root: string; project: ResolvedDshxConfig; cleanup: () => Promise<void> }> {
  ...
  await writeFile(resolve(root, 'package.json'), JSON.stringify(full ? {
    name: '@demo/plugin', type: 'module', exports: { '.': './dist/index.js', './client': './dist/client.js' },
    dsh: { bundle: { patch: './cordis.patch.yml' }, client: { platform: 'web', inject: [], external: [], immediately: false } },
  } : { name: '@demo/plugin', type: 'module', exports: { '.': './dist/index.js' }, dsh: { bundle: { patch: './cordis.patch.yml' } } }, null, 2))
...
describe('add ui scaffold', () => {
  it('adds a contribution to an existing DSHX Client without modifying package metadata', async () => {
    ...
    try {
      ...
      expect(await readFile(resolve(value.root, 'src/client.tsx'), 'utf8')).toContain('slots: [sidebar_footer_actionContribution]')
  ...
  it('supports dry-run, refuses existing generated files, and does not write on provider failure', async () => {
    ...
    try {
      ...
      const created = await createUiScaffold({ project: value.project, slot: 'sidebar.footer.action' }, { compatibility, inspectComposition: async () => inspectResult(), resolveProvider: async () => true, checkManifest: async () => [] })
      expect(created.diagnostics).toEqual([])
      const repeated = await createUiScaffold({ project: value.project, slot: 'sidebar.footer.action' }, { compatibility, inspectComposition: async () => inspectResult(), resolveProvider: async () => true, checkManifest: async () => [] })
