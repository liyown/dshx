...
async function setup(host: 'define' | 'missing' | 'native'): Promise<{ root: string; project: ResolvedDshxConfig; cleanup: () => Promise<void> }> {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: 'demo-plugin', type: 'module', exports: { '.': './dist/index.js' }, dsh: { bundle: { patch: './cordis.patch.yml' } } }),
  )
  ...
  return {
    root,
    project: project(root, host === 'missing' ? undefined : resolve(root, 'src/host.ts')),
...
describe('add hook scaffold', () => {
  it('generates a Cordis hook and attaches it to defineHost setup', async () => {
    ...
    try {
      ...
      const hook = await readFile(resolve(value.root, 'src/hooks/agent.before-request.ts'), 'utf8')
      expect(hook).toContain("import type { Context } from '@becomeopc/dshx/host'")
      expect(hook).toContain('ctx.on("agent.before-request"')
      expect(hook).toContain('...args: unknown[]')
      const host = await readFile(resolve(value.root, 'src/host.ts'), 'utf8')
      expect(host).toContain('from "./hooks/agent.before-request.js"')
  ...
  it('creates a conventional Host when missing and is idempotent', async () => {
    ...
    try {
      ...
      expect(result.diagnostics).toEqual([])
      const repeated = await createHookScaffold(
        { project: { ...value.project, hostEntry: resolve(value.root, 'src/host.ts') }, event: 'agent.ready' },
        { checkManifest: async () => [] },
      )
  ...
  it('uses an existing setup parameter name when attaching the listener', async () => {
    ...
    try {
      await writeFile(
        resolve(value.root, 'src/host.ts'),
  ...
  it('rejects native Hosts, invalid names and rolls back checker failures', async () => {
    ...
    try {
      ...
      const result = await createHookScaffold(
        { project: value.project, event: 'agent.ready' },
        { checkManifest: async () => [{ code: 'DSHX4210', severity: 'error', message: 'bad', file: value.project.packageFile, hint: 'fix' }] },
      )
