...
async function validate(schema: unknown, value: unknown): Promise<unknown> {
  ...
  return result !== undefined && typeof result === 'object' && result !== null && 'value' in result
...
export async function registerApi(
  ctx: Context,
  packageId: string,
  registration: ApiHostRegistration,
): Promise<void> {
...
export function createApiClient<const Methods extends Record<string, ApiMethodDefinition<any, any>>>(
  ...
  const connection = context === undefined || typeof context.get !== 'function'
    ? undefined
    : context.get('connection') as ConnectionLike | undefined
  ...
  for (const name of Object.keys(contract.methods)) {
    ...
    ;(client.safe as Record<string, unknown>)[name] = async (input?: unknown, options?: ApiCallOptions) => {
      try { return { ok: true, value: await invoke(input, options) } } catch (cause) {
