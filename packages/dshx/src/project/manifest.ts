...
function issue(
  code: string,
  severity: DshxDiagnosticSeverity,
  message: string,
  file: string,
  hint: string,
): DshxDiagnostic {
...
function checkStringArray(
  ...
  if (duplicates.length > 0) {
    diagnostics.push(issue(
      code,
      'error',
      `${field} contains duplicate entries: ${duplicates.join(', ')}.`,
      file,
      'Keep each package or module request only once.',
    ))
  }
...
export async function checkProjectManifest(config: ResolvedDshxConfig): Promise<DshxDiagnostic[]> {
  ...
  if (manifest.type !== 'module') {
    diagnostics.push(issue(
      'DSHX4102',
      'error',
      'package.json type must be "module" for the Host ESM artifact.',
      config.packageFile,
