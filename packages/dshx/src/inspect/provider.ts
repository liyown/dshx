...
import type { ProfileOrchestratorOptions, ProjectProfileLink, ResolvedDshInstallation } from '../profile/types.js'
import type { EventSummary, InspectOptions, InspectResult, InspectTarget, ServiceSummary, SlotSummary, ToolSummary } from './types.js'
...
function diagnosticFromError(error: unknown, file: string, fallbackCode = 'DSHX3202') {
  if (error instanceof DshxError) {
    return {
      ...
      hint: error.hint ?? 'Fix the reported runtime problem and try again.',
    }
  }
  return {
    code: fallbackCode,
    ...
    file,
    hint: 'Run with --verbose to inspect the original provider error.',
  }
}
...
function unavailableProvider(file: string) {
  return {
    code: 'DSHX3201',
    severity: 'error' as const,
    message: 'No official DSH Runtime Inspect Provider is available to query from this process.',
    file,
    hint: 'Start a DSH composition with an exposed inspect provider, or use a DSH compatibility adapter that exposes cordisInspect.',
  }
}
...
function absentProfile(project: ResolvedDshxConfig) {
  return {
    ...
    hint: `Run "dshx dev" or "pnpm exec dsh plugin --profile ${project.profile} add ${project.root}" before inspecting the running composition.`,
  }
}
...
function unsupportedTarget(file: string, target: InspectTarget) {
  return {
    code: 'DSHX3204',
    severity: 'error' as const,
    message: `The active DSH Inspect Provider does not support target ${JSON.stringify(target)}.`,
    file,
    hint: 'Use one of the targets supported by the selected DSH compatibility adapter.',
  }
}
...
export function normalizeSlots(value: unknown): readonly SlotSummary[] {
  ...
  return value.map((entry, index) => {
    if (!isRecord(entry) || typeof entry.name !== 'string' || entry.name.trim() === '')
      throw new Error(`Slot Inspect item ${index} must contain a non-empty string name.`)
...
export function normalizeTools(value: unknown): readonly ToolSummary[] {
  ...
  return value.map((entry, index) => {
    if (!isRecord(entry) || typeof entry.name !== 'string' || entry.name.trim() === '')
      throw new Error(`Tool Inspect item ${index} must contain a non-empty string name.`)
...
export function normalizeServices(value: unknown): readonly ServiceSummary[] {
  ...
  return value.map((entry, index) => {
    if (!isRecord(entry) || typeof entry.name !== 'string' || entry.name.trim() === '')
      throw new Error(`Service Inspect item ${index} must contain a non-empty string name.`)
...
export function normalizeEvents(value: unknown): readonly EventSummary[] {
  ...
  return value.map((entry, index) => {
    if (!isRecord(entry) || typeof entry.name !== 'string' || entry.name.trim() === '')
      throw new Error(`Event Inspect item ${index} must contain a non-empty string name.`)
...
export async function inspectProjectComposition(project: ResolvedDshxConfig, target: InspectTarget, options: InspectOptions = {}): Promise<InspectResult> {
  ...
  try {
    ...
    if (dsh.compatibility.inspect !== undefined && !dsh.compatibility.inspect.targets.includes(target)) {
      return {
        ...
        source: 'runtime',
        items: [],
        diagnostics: [unsupportedTarget(project.packageFile, target), ...diagnostics],
        dsh,
      }
    }
    ...
    if (profileLink.state === 'absent')
      return {
        profile: project.profile,
        target,
        source: 'runtime',
        items: [],
        diagnostics: [absentProfile(project), ...diagnostics],
        ...(dsh === undefined ? {} : { dsh }),
        profileLink,
      }
  } catch (error) {
    return {
      ...
      source: 'runtime',
      items: [],
      diagnostics: [diagnosticFromError(error, project.packageFile)],
      cause: error,
      ...(dsh === undefined ? {} : { dsh }),
      ...(profileLink === undefined ? {} : { profileLink }),
    }
  }
  const capability = dsh?.compatibility.inspect?.providerByTarget?.[target] ?? dsh?.compatibility.inspect?.provider
  if (capability === 'unavailable' && options.provider === undefined) {
    return {
      ...
      source: 'runtime',
      items: [],
      diagnostics: [unavailableProvider(project.packageFile), ...diagnostics],
      ...(dsh === undefined ? {} : { dsh }),
      ...(profileLink === undefined ? {} : { profileLink }),
    }
  }
  const provider =
    ...
    (dsh === undefined
      ? undefined
      : new DshInspectBridgeProvider(project, {
          ...(options.env === undefined ? {} : { env: options.env }),
        }))
  ...
  try {
    const method =
      target === 'slots' ? provider.listSlots : target === 'tools' ? provider.listTools : target === 'services' ? provider.listServices : provider.listEvents
    if (typeof method !== 'function')
      return {
        ...
        items: [],
        diagnostics: [
          options.provider === undefined ? unavailableProvider(project.packageFile) : unsupportedTarget(project.packageFile, target),
          ...diagnostics,
        ],
        ...(dsh === undefined ? {} : { dsh }),
        ...(profileLink === undefined ? {} : { profileLink }),
    ...
    let raw: unknown
    try {
      raw = target === 'slots' ? await provider.listSlots(options.slotRoot === undefined ? undefined : { root: options.slotRoot }) : await method.call(provider)
    } catch (error) {
      return {
        profile: project.profile,
        target,
        source: 'runtime',
        items: [],
        diagnostics: [diagnosticFromError(error, project.packageFile), ...diagnostics],
        cause: error,
        ...(dsh === undefined ? {} : { dsh }),
        ...(profileLink === undefined ? {} : { profileLink }),
      }
    ...
    try {
      ...
      return {
        ...
        source: 'runtime',
        items: [],
        diagnostics: [invalidProviderDto(project.packageFile, error), ...diagnostics],
        cause: error,
        ...(dsh === undefined ? {} : { dsh }),
        ...(profileLink === undefined ? {} : { profileLink }),
      }
